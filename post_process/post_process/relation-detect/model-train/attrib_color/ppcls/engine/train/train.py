...
def train_epoch(engine, epoch_id, print_batch_step):
    ...
    for iter_id, batch in enumerate(engine.train_dataloader):
        ...
        #         paddle.to_tensor(batch[0]['car_label'])
        #         paddle.to_tensor(batch[0]['year_label'])
        #         paddle.to_tensor(batch[0]['color_label'])
        #         paddle.to_tensor(batch[0]['brand_label'])
        #     ]
...
def forward(engine, batch):
    if not engine.is_rec:
        batch_output = engine.model(batch[0])
        #pdb.set_trace()
        return batch_output
    else:
