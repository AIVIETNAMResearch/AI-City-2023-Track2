...
class Engine(object):
    def __init__(self, config, mode="train"):
        ...
        self.ema = ExponentialMovingAverage(model=self.model, decay=0.999)
        self.ema.register()
    ...
    def export(self):
        ...
        model.eval()
        out = model(paddle.to_tensor(np.zeros((1,3,224,224),dtype='float32')))
        print("model: ",model)
...
class ExportModel(nn.Layer):
    ...
    def __init__(self, config, model, use_multilabel):
        ...
        #self.multi_branch = model.multi_branch
    ...
    def forward(self, x):
        ...
        if not self.multi_branch:
            ...
            for i in range(head_num):
                output.append(self.out_act(x[i]))
            return output
