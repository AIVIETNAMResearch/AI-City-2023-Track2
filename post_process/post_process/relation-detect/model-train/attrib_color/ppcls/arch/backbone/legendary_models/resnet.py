...
#         self.car_model_fc = Linear(
#             self.avg_pool_channels,
#             767,
#             weight_attr=ParamAttr(initializer=Uniform(-stdv, stdv)))
        
        for i in range(len(self.class_num)):
            exec("self.fc_%d = Linear(self.avg_pool_channels,self.class_num[%d],weight_attr=ParamAttr(initializer=Uniform(-stdv, stdv)))" % (i, i))
    ...
    def forward(self, x):
        with paddle.static.amp.fp16_guard():
            ...
            #print(x.shape)
            fea = F.normalize(x,axis=1)
            res = []
            out0 = self.fc_0(x)
            res.append(out0)
            
            #for i in range(len(self.class_num)):
            #    exec("out%d = self.fc_%d(x)" % (i, i))
            #    exec("res.append(out%d)" % i)
            return res
