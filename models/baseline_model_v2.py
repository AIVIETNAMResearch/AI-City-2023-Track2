...
import timm
from .aggregation_head import ContextualizedWeightedHead, FCNet, MeanHead
import torch
...
class VideoTextFeatureExtractor(nn.Module):
    def __init__(self, base_setting, text_head_setting, vision_head_setting, device=torch.device('cpu')):
        ...
        self.text_head = MeanHead(device=device)
        self.vision_head = MeanHead(device=device)
    ...
    def compute_video(self, frames, motion, motion_line):
        ...
        agg_motion_features = torch.mean(motion_features, motion_line_features)
